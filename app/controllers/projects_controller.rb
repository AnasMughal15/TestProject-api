class ProjectsController < ApplicationController
  before_action :set_project, only: [ :show, :update, :destroy, :assign_developer, :members ]
  before_action :authorize_manager, only: [ :create, :update, :destroy ]
  before_action :check_developers_assigned, only: [ :create ]
  def index
    result = ProjectsService.fetch_projects(current_user, params)
  ...
  def members
    users = User.joins(:project_users).where(project_users: { project_id: @project.id })
    users = users.where(user_type: params[:user_type]) if params[:user_type].present?
    render json: users.select(:id, :name, :email, :user_type), status: :ok
  end

  def statuses
    render json: Project.statuses.map { |label, id| { id: id, label: label } }, status: :ok
  end

  def available_developers
    @developers = User.where(user_type: "developer").left_joins(:projects).where(projects: { id: nil })
    @developers = @developers.where("LOWER(users.name) LIKE ?", "%#{params[:search].downcase}%") if params[:search].present?
    render json: @developers.select(:id, :name, :email), status: :ok
  end
  ...
  def project_params
    params.permit(:name, :description, :status, developer_ids: [])
  end
